...
class DSDGEN() extends DataGenerator {
  def generate(sparkContext: SparkContext, name: String, partitions: Int, scaleFactor: String) = {
    val generatedData = {
      sparkContext.parallelize(1 to partitions, partitions).flatMap { i =>
        val toolsPath = SparkFiles.get("tools")
        val localToolsDir = if (new java.io.File(s"$toolsPath/dsdgen").exists) {
          toolsPath
        } else {
          sys.error(s"Could not find dsdgen at $toolsPath/dsdgen.")
        }
